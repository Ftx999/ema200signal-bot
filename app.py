...
def scan_symbols():
    ...
    try:
        ...
        raw_pairs = [s for s in markets if 'USDT' in s and markets[s]['type'] == 'swap']
        usdt_pairs = list(set(raw_pairs))  # ✅ 移除重複
        usdt_pairs.sort()  # Optional: 讓輸出有序
    except Exception as e:
        print(f"❌ 無法載入市場資料: {e}")
        return
